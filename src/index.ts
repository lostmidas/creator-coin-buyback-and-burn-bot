...
import { TierEngine } from './services/calculator';
import { SwapService } from './services/swap';
import { loadState, saveState, AppState } from './utils/state';

async function main() {
  const dryRun = process.argv.includes('--dry-run');
  if (dryRun) console.log('>>> DRY RUN MODE ENABLED: No transactions will be sent. State will not be saved. <<<');
